var Busboy, DataSource, LargeObjectManager, PostgresStorage, Promise, _, async, debug, getDefaultSettings, pg;
...
PostgresStorage = (function() {
  ...
  PostgresStorage.prototype.destroyContainer = function(name, callback) {
    ...
    self = this;
    currentClient = null;
    closeConnection = function(error, success) {
      if (currentClient != null) {
        currentClient.release();
      }
      return callback(err, res);
    };
    return self.db.connect().then(function(client) {
  ...
  PostgresStorage.prototype.getFileById = function(currentClient, id, callback) {
    return currentClient.query("select * from " + this.settings.table + " where id = $1", [id], function(err, res) {
      if (err) {
        return callback(err);
      }
